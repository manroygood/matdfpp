...
elseif strcmp(action,'proceed')
   ...
   sud.o = sud.c;
   set(ppset,'UserData',sud);
   he = findobj('name','matpplane Equilibrium point data');
   hl = findobj('name','matpplane Linearization');
   close([he;hl]);
   % set([he;hl],'vis','off');
...
%       set([arr.hx;arr.hy;arr.barrows],'vis','on');
     ...
     % Find matpplane Display if it exists.	
     % If matpplane Display exists, update it.  If it does not build it.
	 ...
	 if ~isempty(menull)
       ...
       ppdisp = figure('name','matpplane Display',...
		       'interrupt','on',...
		       'visible','off',...
		       'tag','matpplane');
       matpplane('figdefault',ppdisp);
       dud = get(ppdisp,'UserData');
       dud.syst = sud.c;
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       dbutt(2) = uicontrol('style','push',...
			    ...
			    'string','Quit',...
			    'CallBack','matpplane(''quit'')','visible','off');
       
       dbutt(3) = uicontrol('style','push',...
			    'pos',[buttl,ppaxb+buttsep,buttw,butth],...
			    'string','Print',...
			    'CallBack','matpplane(''print'')','visible','off');	
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
	   ...
	   'CallBack','matpplane(''showbar'')');
       
       set(0,'showhiddenhandles',hhsetup);
       
       set(gcf,'WindowButtonDownFcn','matpplane(''down'')');
       set(ppdisp,'WindowButtonMotionFcn','matpplane(''cdisp'')');
	...
	case 6
     ...
     % Initialize important information as UserData data.
     ...
     ppkbd = findobj('name','matpplane Keyboard input','vis','on');
     if ~isempty(ppkbd),matpplane('kbd'),end
...
elseif strcmp(action,'linear')
   ...
   % Find matpplane Linearization if it exists.	
   % If matpplane Linearization exists, update it.  If it does not build it.
   
   pplin= findobj('name','matpplane Linearization');
   if (~isempty(pplin))
      figure(pplin);
      ...
      pplin = figure('name','matpplane Linearization',...
         'numb','off',...
         'interrupt','on',...
         'visible','off',...
         'tag','matpplane');
      matpplane('figdefault',pplin);
      dud = get(pplin,'UserData');
      dud.dir = 0;
      dud.syst = system;
      dud.settings = settings;
      ...
      fundCallBack = ['dud = get(gcf,''UserData'');',...      
		  'col = dud.color.sep;',...
		  'vect = dud.vectors;',...
		  'h = zeros(1,2);',...
      ...
      dud.menu = uimenu(solmenu,'CallBack',fundCallBack);
...
elseif strcmp(action,'dirfield')
   ...
   % Find matpplane Display and get the UserData data.
   ...
   % NPK attempt to setup the notice window for linearization:
   %    nframe = uicontrol('style','frame','visible','on');
   nstr = {'More';'than';'five';'lines';'of text'};
   dud.notice = uicontrol('style','text',...
       'horiz','left',...
   ...
   N = dud.syst.npts;
   if strcmp(get(disph,'name'),'matpplane Linearization') 
      N = floor(3*N/4);
   end    
...
elseif strcmp(action,'hotcold')
   ...
   % 'hotcold' is the CallBackback for the menu selection that makes the
   ...
   if (findstr(get(mehc,'label'),'inactive'))
      ...
      set(ppdisp,'WindowButtonDownFcn','matpplane(''down'')');
...
elseif strcmp(action,'down')
  ...
  % 'down' is the Window Button Down CallBack.  It starts the computation of
  ...
  matpplane('solution',initpt,disph);
...
elseif strcmp(action,'several')
   ...
   % 'several' allows the UserData to pick several initial points at once.
...
elseif strcmp(action,'test case')
   ...
   for k = 1:10
      initpt = k*[-.2 .2];
      matpplane('solution',initpt,ppdisp);
      initpt = k*[-.2 -.2];
      matpplane('solution',initpt,ppdisp);
   end  
   matpplane('solution',[1,-1],ppdisp);
   dud = get(gcf,'UserData');
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5,1} = 'Ready.';
...
elseif strcmp(action,'kcompute')
   ...
   % 'kcompute' is the CallBack back for the Compute 
   % button on the matpplane Keyboard figure.
...
elseif strcmp(action,'kbd')
   ...
   % 'kbd' is the CallBackback for the Keyboard Input menu selection.  It 
   % sets up the matpplane Keyboard figure which allows accurate input of 
   ...
   ppkbd = findobj('name','matpplane Keyboard input');
   if  ~isempty(ppkbd)
     delete(ppkbd);
   end
         
   ppkbd = figure('name','matpplane Keyboard input',...
		  'vis','off',...
		  'numb','off','tag','matpplane');
   
   matpplane('figdefault',ppkbd);
   ...
   kbd.comp = uicontrol('style','push',...		
			'string','Compute','CallBack','matpplane(''kcompute'')');
   ...
   set(kbd.spec,'CallBack',specCallBack);
   set(ppkbd,'resize','on');
...
%  z0 = ppginput(1);
  ...
  ppeqpt = findobj('name','matpplane Equilibrium point data');
  
   if (isempty(ppeqpt))
     ppeqpt = figure('vis','off','resize','on',...
 		    'name','matpplane Equilibrium point data',...
 		    'numb','off','tag','matpplane');
     matpplane('figdefault',ppeqpt);
...
elseif strcmp(action,'zoomin')
   ...
   % 'zoomin' is the CallBackback for the Zoomin menu item.  It allows the
   % UserData to pick a new display rectangle.
   
   set(gcf,'WindowButtonDownFcn','matpplane(''zoom'')',...
      'WindowButtonUpFcn','1;','Interrupt','on');
   set(gca,'Interrupt','on');
   dud = get(gcf,'UserData');
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
...
elseif strcmp(action,'zoom')
   ...
   set(disph,'UserData',dud);
   set(disph,'WindowButtonDownFcn','matpplane(''down'')',...
      'WindowButtonUpFcn','');
   matpplane('dirfield',disph);
   ppset = findobj('name','matpplane Setup');
   if isempty(ppset)
      matpplane('confused');
   else
...
elseif strcmp(action,'showbar')
  ...
  if strcmp(state,'figure')
    ...
    switch name
     case 'matpplane Display'
      set(findobj(sbfig,'tooltipstr','Zoom Out'),...
	  'clickedCallBackback',['matpplane(''zoomback'');' fixtb]);
      set(findobj(sbfig,'tooltipstr','Zoom In'),...
	  'clickedCallBackback',['matpplane(''zoomin'');' fixtb]);
      delete(findobj(sbfig,'tooltipstr','Rotate 3D'));
      
     case 'matpplane Linearization'
...
elseif strcmp(action,'dall')
   ...
   % 'dall' is the CallBackback for the Erase all graphics objects.  
...
elseif strcmp(action,'dallsol')
   ...
   % 'dallsol' is the CallBackback for the Erase all solutions option.  
...
elseif strcmp(action,'dallep')
   ...
   % 'dallep' is the CallBackback for the Erase all equilibrium points option.  
...
elseif strcmp(action,'dalllev')
   ...
   % 'dalllev' is the CallBackback for the Erase all level curves option.  
...
elseif strcmp(action,'dallics')
   ...
   % 'dallics' is the CallBackback for the Erase all marked initial cond.
...
elseif strcmp(action,'settings')
   ...
   setfig = findobj('name','matpplane Settings');
   if ~isempty(setfig)
     delete(setfig);
   end
   
   setfig = figure('name','matpplane Settings',...
		   'numb','off',...
		   'tag','matpplane','vis','off');
   
   matpplane('figdefault',setfig);
   ...
   data.nstep = uicontrol('style','edit','CallBack',nstepCallBack,...
      'string',num2str(data.settings.refine));
   
   if strcmp(solver,'Runge-Kutta 4')
     rtolstr = 'Step size: ';
     rtolentry = num2str(data.settings.stepsize);
     rtolCallBack =['data = get(gcf,''UserData'');',...
   ...
   rtol = uicontrol('style','text','horiz','left',...
       'string',rtolstr); 
   data.rtol = uicontrol('style','edit','CallBack',rtolCallBack,...
      'string',rtolentry);
   
   kk = 1+2*data.settings.magn;
   magCallBack = ['data = get(gcf,''UserData'');',...
         'mag = 	(str2num(get(data.mag,''string''))-1)/2;',...
         'data.settings.magn = max(0,mag);',...
         'set(gcf,''UserData'',data);'];
   
    speedCallBack = ['data = get(gcf,''UserData'');',...
		 'me = data.speed;',...
   ...
   data.mag = uicontrol('style','edit','CallBack',magCallBack,...
      'string',num2str(kk));
...
elseif strcmp(action,'setchange')
   ...
   % 'setchange' is the CallBackback for the Change button 
   % on the matpplane Settings window.
...
elseif strcmp(action,'delete')
   ...
   % 'delete' is the CallBackback for the Delete a graphics object selection
   % on the menu.
...
elseif strcmp(action,'text')
   ...
   ppdisp = gcf;
   oldCallBack = get(ppdisp,'WindowButtonDownFcn');
   set(ppdisp,'WindowButtonDownFcn','');
   ...
   set(ppdisp,'WindowButtonDownFcn',oldCallBack);
...
elseif strcmp(action,'plxy')
   ...
   switch type
    case 1 
      ...
      ylabel(xstring)
      set(r3dh,'vis','off')
      set(fig,'WindowButtonDownFcn','matpplane(''plxybdf'')');
    case 2
...
elseif strcmp(action,'crop')
   ...
   hh = matpplane('plotxyfig',type,ppxy);
   ud = get(hh(1),'UserData');
   ud.data = [t;x;y];
   set(hh(1),'UserData',ud);
   aud = get(hh(2),'UserData');
...
elseif strcmp(action,'plxybdf')
   ...
   dispa = gca;
   dispf = gcf;
   set(dispf,'windowbuttonmotionfcn','matpplane(''plxybmf'')',...
      'windowbuttonupfcn','matpplane(''plxybuf'')',...
      'Interrupt','on');
...
elseif strcmp(action,'print')
   ...
   nstr = get(dud.notice,'string');
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Preparing to print the matpplane Display Window.  Please be patient.';
   set(dud.notice,'string',nstr);
   
   nstr(1:4) = nstr(2:5);
   nstr{5} = 'Printing the matpplane Display Window.';
   set(dud.notice,'string',nstr);
...
elseif strcmp(action,'zoominsq')
   ...
   % 'zoominsq' is the CallBackback for the Zoom in square menu item.  
   
   set(gcf,'WindowButtonDownFcn','matpplane(''zoomsqd'')',...
      'WindowButtonUpFcn','1;','Interrupt','on');
   set(gca,'Interrupt','on');
   dud = get(gcf,'UserData');
   nstr = get(dud.notice,'string');
...
elseif strcmp(action,'zoomsqd')
   ...
   set(ppdispa,'UserData',aud);
   set(gcf,'windowbuttonmotionfcn','matpplane(''zoomsqm'')',...
      'windowbuttonupfcn','matpplane(''zoomsqu'')');
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'UserData',dud);
   set(disph,'WindowButtonDownFcn','matpplane(''down'')',...
      'WindowButtonMotionFcn','matpplane(''cdisp'')',...
      'WindowButtonUpFcn','');
   matpplane('dirfield',disph);
   ppset = findobj('name','matpplane Setup');
   if isempty(ppset)
...
elseif strcmp(action,'level')
  ...
  pplevel = figure('name','matpplane Level sets',...
		 'vis','off',...
		 'numb','off','tag','matpplane');
  
  matpplane('figdefault',pplevel);
...
elseif strcmp(action,'closefcn')
   ...
   if strcmp(name,'matpplane Setup') | strcmp(name,'matpplane Display')
      ...
      butt = questdlg(quest,'Quit matpplane?','Quit','Cancel','Quit');
      if strcmp(butt,'Quit')
         matpplane('quit');
      end
...
elseif strcmp(action,'confused')
   ...
   qstring = {['You will have to restart matpplane from '...
      ...
      'What do you want to do?'};
   bstr1 = 'Quit and restart matpplane.';
   bstr2 = 'Just quit matpplane.';
   bstr3 = 'Do nothing.';
   answer = questdlg(qstring,tstring,bstr1,bstr2,bstr3,bstr1);
   if strcmp(answer,bstr1)
      delete(findobj('tag','matpplane'));
      matpplane;return
...
elseif strcmp(action,'export')
  ...
  % export is the CallBackback for the Export solution data item in the
  % Options menu.
